...
process.env.DATABASE_URL =
  process.env.TEST_DATABASE_URL ?? "postgresql://ecommerce:ecommerce@localhost:15432/ecommerce_test_db?schema=public";
...
// The checkout tests are written against the mock gateway. These are set
// empty rather than deleted on purpose: dotenv skips keys already present on
// process.env, so an empty value blocks the developer's real .env key from
// loading, while still reading as falsy when the gateway is chosen. Deleting
// them would invite dotenv to fill them back in and send the suite to Stripe.
